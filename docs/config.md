...
| `enable_alert_popup`              | Show a dismissible popup for warnings/errors that are otherwise only logged.                        | `true`                                                                 |
| `alert_popup_min_level`           | Minimum severity raising an alert popup: `Warn` or `Error`.                                         | `Warn`                                                                 |
...
- Alerts are raised from log events, so they are subject to the application's logging setup: `RUST_LOG=off` disables them, and an alert stays visible on the logs page after being dismissed. Repeats of the same event coalesce into one alert with a count, and at most 20 alerts are kept. See the [Alerts section of the README](../README.md#alerts) for the popup's keys.
